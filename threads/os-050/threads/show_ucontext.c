...
void get_start_end(long *startp, long *endp) {
    char filename[128];
    char line[256];
    FILE *f;
...
int main(int argc, char **argv) {
    ...
    err = getcontext(&mycontext);  //* getcontext() return 0 on success
    assert(!err);
    ...
    printf(
        "memory address of the program counter (RIP) saved "
        "in mycontext = 0x%lx\n",
    ...
    printf("memory address of the variable err = %p\n",
           (void *)&err);  //*
    printf(
        "number of bytes pushed to the stack between setcontext_called "
        "and err = %ld\n",
    ...
    printf(
        "number of bytes between err and the saved stack in mycontext "
        "= %ld\n",
